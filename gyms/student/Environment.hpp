...
class ENVIRONMENT : public RaisimGymEnv {
 ...
 public:
  ...
  explicit ENVIRONMENT(const std::string& resourceDir, const Yaml::Node& cfg, bool visualizable) :
    ...
    stateHistScaling_ = std::make_shared<ScalingAndOffset<float>>(resourceDir_+"/scaling/state_hist.yaml");
    actionScaling_ = std::make_shared<ScalingAndOffset<float>>(resourceDir_+"/scaling/action.yaml");
    stateHistObservation_ = std::make_shared<HistoryObservation<float, 4, 100>>(robotState_.get(), command_.get(), footMotion_.get(), actuator_.get(), stateHistScaling_.get());
  ...
  float step(const Eigen::Ref<EigenVec>& action) final {
    ...
    if (it_ % 2 == 0) { //Update History at 50Hz => 2s history.
      stateHistObservation_->advance();
    }
